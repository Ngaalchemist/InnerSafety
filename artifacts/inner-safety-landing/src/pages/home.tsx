  ...
  return (
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative w-full overflow-hidden bg-background">
        {/* Full-bleed tree image on the right — the source image already has its own
            light→dark (sky→soil) gradient baked in, so we no longer paint a separate
            dark backdrop div; that div was what swallowed the sub-headline text before. */}
        <div className="absolute inset-y-0 right-0 w-full md:w-[58%] lg:w-[52%] z-0">
          <img
            src={heroImg}
            alt="Sacred tree with glowing roots"
            className="w-full h-full object-cover object-center opacity-25 md:opacity-100"
          />
          {/* Soft fade so the image blends into the background behind the text column,
              instead of a hard seam */}
          <div className="hidden md:block absolute inset-y-0 left-0 w-40 bg-gradient-to-r from-background to-transparent" />
        </div>

        <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-16 md:pt-40 md:pb-24 min-h-[92dvh] flex items-center">
          {/* Text content — confined to the left column, always over the light
              background/gradient edge, never over the image's dark soil area */}
