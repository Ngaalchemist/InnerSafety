...
export default function Home() {
  ...
  return (
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      {/* SECTION 1: HERO
          Full-bleed CLEAN photo (tree + roots only, no baked-in text — generated
          separately) as the single background. Every piece of copy, the stat
          callouts, and the bottom feature row are now real HTML overlaid on top,
          positioned to match the light sky zone (top) vs dark soil zone (bottom)
          of the photo — instead of relying on a flattened mockup image. */}
      <section className="relative w-full overflow-hidden bg-background min-h-[100dvh] flex flex-col">
        <img
          src={heroImg}
          alt="Cây cổ thụ với bộ rễ phát sáng, tượng trưng cho nền tảng an toàn bên trong"
          className="absolute inset-0 w-full h-full object-cover object-center"
        />
        {/* Left-side scrim so text stays readable regardless of what's under it,
            without hiding the photo entirely */}
        <div className="absolute inset-0 bg-gradient-to-r from-background/95 via-background/40 to-transparent md:from-background/90 md:via-background/20 md:to-transparent" />
        {/* Extra dark scrim over just the bottom third (soil area) so white text/icons
            there stay legible even where the left gradient above has faded out */}
        <div className="absolute inset-x-0 bottom-0 h-[38%] bg-gradient-to-t from-[#0f0b08]/85 to-transparent" />

        <div className="relative z-10 max-w-7xl mx-auto w-full px-6 pt-28 md:pt-32 flex-1 flex flex-col">
          {/* Upper block — sits on the light sky zone, dark readable text */}
          ...
          {/* Lower block — sits on the dark soil zone, white text (mirrors the
              "10%/90%" + checklist treatment from the reference mockup) */}
          <div className="max-w-xl pb-10 md:pb-14">
            ...
            {/* Feature icon row — recreated as real markup instead of baked pixels,
                so it reflows properly on mobile instead of being cropped */}
            <div className="flex flex-wrap gap-x-6 gap-y-3 text-xs text-white/70 border-t border-white/15 pt-6">
              <span className="flex items-center gap-1.5"><PlayCircle className="w-4 h-4 text-primary" /> Học online mọi lúc, mọi nơi</span>
              <span className="flex items-center gap-1.5"><PlayCircle className="w-4 h-4 text-primary" /> Truy cập ngay không cần chờ đợi</span>
              <span className="flex items-center gap-1.5"><Shield className="w-4 h-4 text-primary" /> Có người đồng hành trong suốt hành trình</span>
              <span className="flex items-center gap-1.5"><Lock className="w-4 h-4 text-primary" /> Truy cập trọn đời</span>
              <span className="flex items-center gap-1.5"><Shield className="w-4 h-4 text-primary" /> Hoàn tiền 7 ngày</span>
            </div>
          </div>
        </div>

        {/* Stat callouts, positioned over the tree on the right — like the "10% / 90%"
            labels in the reference mockup. Hidden on small screens since the photo
            is mostly cropped out of view there. */}
        <div className="hidden lg:flex flex-col absolute right-10 top-[18%] z-10 text-right">
          <span className="text-4xl font-serif text-primary">10%</span>
          <span className="text-xs text-foreground/70 max-w-[10rem] mt-1">Những gì người khác nhìn thấy ở bạn.</span>
        </div>
        <div className="hidden lg:flex flex-col absolute right-10 top-[58%] z-10 text-right">
          <span className="text-4xl font-serif text-primary">90%</span>
          <span className="text-xs text-white/70 max-w-[10rem] mt-1">Những gì thực sự quyết định cuộc đời bạn.</span>
        </div>
      </section>
