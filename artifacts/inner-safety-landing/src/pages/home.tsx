...
import heroImg from '@assets/hero_tree_cliff.jpg'; // full-bleed hero background — tree/cliff cutaway with 10%/90% stat labels already baked in on the right (re-compressed as .jpg to fix washed-out PNG palette compression)
// heroAltImg removed — it pointed to a file no longer present in attached_assets and was never
// actually rendered anywhere in this component, so it was only breaking the build for no visual benefit.
...
// Icons
import { CheckCircle2, PlayCircle, Shield, ChevronRight, Lock, MessageCircle, Zap, Users } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <div className="bg-background text-foreground overflow-x-hidden">
      ...
      {/* SECTION 1: HERO — no scrim; image shown clean. Text uses shadow only. */}
      <section className="relative w-full overflow-hidden bg-background min-h-[100dvh] flex flex-col">
        ...
        {/* Above-ground zone — eyebrow + headline own this entire area (roughly
            up to the horizon in the photo). pt pushed down so the transparent
            nav/logo above never overlaps the eyebrow. */}
        <div className="relative z-10 w-full min-h-[46vh] md:min-h-[48vh] flex flex-col justify-center px-[5%] md:px-[6%] pt-16 md:pt-20">
          <div>
            <div
              className="font-serif uppercase tracking-widest text-xs font-bold mb-4"
              style={{ color: '#6b4e14', textShadow: '0 1px 10px rgba(255,255,255,0.7)' }}
            >
              Inner Safety Experience™
            </div>
            <h1
              className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold leading-snug"
              style={{ textShadow: '0 2px 18px rgba(255,255,255,0.55)' }}
            >
              <span style={{ color: '#5b3a1e' }}>Sau một biến cố...</span><br/>
              <span className="md:whitespace-nowrap" style={{ color: '#5b3a1e' }}>cuộc sống của bạn có thể đã tiếp tục.</span><br/>
              <span style={{ color: '#8a6a1f' }}>Nhưng bên trong,</span><br/>
              <span className="md:whitespace-nowrap">
                <span style={{ color: '#8a6a1f' }}>bạn vẫn chưa thật sự</span>{' '}
                <span style={{ color: '#5b3a1e' }}>cảm thấy an toàn</span>
              </span>
            </h1>
          </div>
        </div>

        {/* Below-ground zone — subheadline + bullets + CTA + badges. Positioned
            absolutely at top:50% for a clear gap below the headline, instead of
            following immediately in normal flow. */}
        <div className="absolute z-10 w-full px-[5%] md:px-[6%]" style={{ top: '50%' }}>
          <div className="max-w-2xl">
            <p
              className="text-lg font-semibold mb-8 max-w-lg"
            ...
            <ul className="space-y-3 mb-8" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 400 }}>
              <li className="flex items-start gap-3 text-white" style={{ textShadow: '0 1px 10px rgba(0,0,0,0.55)' }}>
                <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                <span className="md:whitespace-nowrap text-sm md:text-base"><strong className="text-primary font-extrabold">Làm dịu hệ thần kinh</strong> để cơ thể không còn luôn trong trạng thái cảnh giác</span>
              </li>
              <li className="flex items-start gap-3 text-white" style={{ textShadow: '0 1px 10px rgba(0,0,0,0.55)' }}>
                <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                <span className="md:whitespace-nowrap text-sm md:text-base"><strong className="text-primary font-extrabold">Ngủ sâu hơn, bớt overthinking</strong> và lấy lại cảm giác bình yên trong chính mình</span>
              </li>
              <li className="flex items-start gap-3 text-white" style={{ textShadow: '0 1px 10px rgba(0,0,0,0.55)' }}>
                <CheckCircle2 className="w-5 h-5 text-primary mt-0.5 flex-shrink-0" />
                <span className="md:whitespace-nowrap text-sm md:text-base"><strong className="text-primary font-extrabold">Đưa ra quyết định từ sự vững vàng</strong> thay vì phản ứng vì nỗi sợ</span>
              </li>
            </ul>

            {/* CTA — widened ~20% from before (420px → ~504px) */}
            <Button 
              onClick={scrollToRegister}
              className="w-full max-w-[504px] h-14 rounded-md text-primary-foreground text-lg font-bold tracking-wide border border-[#f5dfa0]/60 transition-all hover:scale-[1.02]"
              style={{
                background: 'linear-gradient(180deg, #f4e4b0 0%, #d4af37 35%, #a9791f 70%, #8a6318 100%)',
                boxShadow: '0 4px 20px rgba(154,115,25,0.45), inset 0 1px 0 rgba(255,255,255,0.4)',
              }}
            >
              TÔI MUỐN XÂY LẠI GỐC RỄ CỦA MÌNH
            </Button>

            {/* Trust badges — single line, same width as the button */}
            <div
              className="flex flex-nowrap items-center justify-between gap-2 mt-5 w-full max-w-[504px] text-[11px] md:text-xs text-white/90 font-medium overflow-x-auto"
              style={{ textShadow: '0 1px 8px rgba(0,0,0,0.65)' }}
            >
              <span className="flex items-center gap-1 whitespace-nowrap"><PlayCircle className="w-3.5 h-3.5 text-primary shrink-0" /> Học online</span>
              <span className="flex items-center gap-1 whitespace-nowrap"><Zap className="w-3.5 h-3.5 text-primary shrink-0" /> Bắt đầu ngay</span>
              <span className="flex items-center gap-1 whitespace-nowrap"><Users className="w-3.5 h-3.5 text-primary shrink-0" /> Có người đồng hành</span>
              <span className="flex items-center gap-1 whitespace-nowrap"><Lock className="w-3.5 h-3.5 text-primary shrink-0" /> Truy cập trọn đời</span>
              <span className="flex items-center gap-1 whitespace-nowrap"><Shield className="w-3.5 h-3.5 text-primary shrink-0" /> Hoàn tiền 7 ngày</span>
            </div>
          </div>
        </div>

        {/* Quote — now anchored right at the horizon (trunk/root junction), with a
            horizontal connector growing out from the trunk toward the box on the
            right, instead of floating up in the sky */}
        <div className="hidden lg:flex absolute right-[6%] top-[49%] -translate-y-1/2 z-10 items-center max-w-[22rem]">
          <div className="flex items-center shrink-0">
            <div className="w-2 h-2 rounded-full bg-primary/80 shrink-0" />
            <div className="h-px w-16 md:w-24 bg-gradient-to-r from-primary/70 to-primary/10" />
          </div>
          <div className="bg-[#0f0b08]/55 backdrop-blur-[2px] rounded-xl px-5 py-4 border-l border-primary/30">
            <p className="text-white text-sm font-serif italic leading-relaxed">
              <span className="whitespace-nowrap block">"Bình an không được xây từ những gì xảy ra bên ngoài.</span>
              <span className="whitespace-nowrap block">Nó được nuôi dưỡng từ bộ rễ bên trong."</span>
            </p>
          </div>
        </div>
      </section>
  ...
  return (
    <nav className="absolute top-0 left-0 w-full z-20 py-6">
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <div
          className="font-serif font-bold text-base flex items-center gap-2 text-foreground"
          style={{ textShadow: '0 1px 8px rgba(255,255,255,0.7)' }}
        >
          <img src={ngaLogo} alt="Nga Alchemist" className="h-5 w-auto object-contain" />
          Nga Alchemist
        </div>
        
        <div className="hidden md:flex items-center gap-8 text-sm font-medium">
          <div className="flex gap-6 text-foreground/90" style={{ textShadow: '0 1px 8px rgba(255,255,255,0.7)' }}>
          ...
          <Button onClick={onRegisterClick} size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-5 h-8 text-sm transition-transform hover:scale-105">
            ĐĂNG KÝ NGAY
          </Button>
        </div>
